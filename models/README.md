...
Windows 桌面版的实际模型包根目录固定为 `D:\open-tts\models`。完整懒人包、运行时、权重、配置和缓存均应保留在各模型自己的子目录中，避免模型文件分散在项目外部磁盘。

源码中的 `models/` 只保留随安装包提供的微小实时 VAD 构建资源；程序第一次启动会将它复制到 `D:\open-tts\models\realtime`，实际运行始终优先使用 D 盘资源库。
