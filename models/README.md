...
Windows 桌面版使用单一的 `D:\open-tts` 根目录：程序默认安装在 `D:\open-tts\OpenTTS Studio`，实际模型包固定在 `D:\open-tts\models`，配置、任务和输出固定在 `D:\open-tts\data`。完整懒人包、运行时、权重、配置和缓存均应保留在各自的受管子目录中，避免模型文件分散在项目外部磁盘。

源码中的 `models/` 只保留随安装包提供的微小实时 VAD 构建资源；实际运行始终优先使用 `D:\open-tts\models\realtime`，仅在该受管目录尚未就绪时回退到安装包内的只读资源。
...
这些文件可能很大，且通常带有各自的授权条款，因此整个目录已被 Git 忽略。不要把模型权重、Python 运行时或生成的音频提交到仓库。卸载 `D:\open-tts\OpenTTS Studio` 时不会触碰同级的 `models` 与 `data` 目录。
