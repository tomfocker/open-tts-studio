...
- 本地语音增强对比：DeepFilterNet3 与 MossFormer2_SE_48K 使用可选的独立 Python 运行时，串行清理同一条音视频素材并保留原文件；不上传媒体或自动下载模型。
- 桌面端音色库：可将本地参考音频或生成结果一键加入音色库。
- 参考音频导入后会立即在后台识别原文；识别完成后自动恢复当前语音模型预热，减少首次生成等待。
- Qwen3-ASR 与最终音频强制对齐优先使用独立的 NVIDIA CUDA 运行时；未安装时自动回退 DirectML/Vulkan，再回退 CPU，不影响任何 TTS 模型的加载。
