...
function BilibiliDownloadPanel({ onClose, onDownloaded }: { onClose: () => void; onDownloaded: () => Promise<void> }) {
  ...
  return <section className="mediaSamplerDownloader" aria-label="B站视频下载">
    ...
    {(notice || error) && <p role={error ? "alert" : "status"} aria-live={error ? "assertive" : "polite"} className={error ? "error" : ""}>{error ?? notice}</p>}
  </section>;
}
    ...
    <section className={downloaderOpen ? "mediaSamplerWorkspace withDownloader" : "mediaSamplerWorkspace"}>
      ...
      <main className="mediaSamplerGrid">
        <section className="mediaSamplerPreview">
          ...
          </> : <div className="mediaSamplerEmpty"><History size={32} /><strong>还没有本地下载视频</strong><span>从 B 站下载完成后，视频会自动加入本机历史。</span><button type="button" className="primary" onClick={() => setDownloaderOpen(true)}><Download size={15} />从 B 站下载</button></div>}
        </section>
        <aside className="mediaSamplerSide">
      <section className="mediaSamplerHistorySection"><div className="mediaSamplerSectionTitle"><span><History size={16} />下载历史</span><button className="icon" type="button" title="刷新历史" aria-label="刷新下载历史" disabled={Boolean(pending)} onClick={() => void refresh()}><RefreshCw size={15} /></button></div><div className="mediaSamplerHistory">{history.map((entry) => <article key={entry.id} className={entry.id === selected?.id ? "active" : ""}><button type="button" className="mediaSamplerHistoryOpen" aria-pressed={entry.id === selected?.id} disabled={Boolean(pending) || !entry.exists} onClick={() => void openHistory(entry)}><span><strong title={entry.title ?? entry.itemTitle ?? "B 站视频"}>{entry.title ?? entry.itemTitle ?? "B 站视频"}</strong><small>{entry.videoQuality?.label ?? "MP4"} · {formatBytes(entry.fileSizeBytes)} · {formatDate(entry.downloadedAt)}</small></span>{pending === `open:${entry.id}` && <Loader2 className="spin" size={14} />}</button><button className="mediaSamplerHistoryDelete" type="button" title="移除下载记录（不删除本地 MP4）" aria-label={`移除下载记录 ${entry.title ?? entry.itemTitle ?? "B 站视频"}`} disabled={Boolean(pending)} onClick={() => void removeHistory(entry)}><Trash2 size={14} /></button></article>)}{!history.length && <p>下载记录保存在本机；不会保存账号 Cookie、CDN 地址或参考音频。</p>}</div></section>
          <section className="mediaSamplerClip"><div className="mediaSamplerSectionTitle"><span><Scissors size={16} />已选片段</span>{selected && <small>{selectedDuration.toFixed(1)} 秒</small>}</div>{selected ? <><div className="mediaSamplerRangeFields"><label>开始<input disabled={Boolean(pending)} type="number" min="0" max={Math.max(0, range[1] - 0.1)} step="0.1" value={range[0].toFixed(1)} onChange={(event) => updateRange(Number(event.target.value), range[1])} /></label><label>结束<input disabled={Boolean(pending)} type="number" min={Math.min(duration, range[0] + 0.1)} max={duration || undefined} step="0.1" value={range[1].toFixed(1)} onChange={(event) => updateRange(range[0], Number(event.target.value))} /></label></div><label className="mediaSamplerSampleName">音色名称<input disabled={Boolean(pending)} value={sampleName} maxLength={80} onChange={(event) => setSampleName(event.target.value)} /></label><div className="mediaSamplerActions"><button type="button" disabled={Boolean(pending) || selectedDuration < 0.1} onClick={previewSelection}><Play size={15} />试听片段</button><button className="primary" type="button" disabled={Boolean(pending) || selectedDuration < 0.1} onClick={() => void createVoiceFromSelection}>{pending === "extract-voice" ? <Loader2 className="spin" size={15} /> : <Scissors size={15} />}提取并入库</button></div></> : <p>打开本地视频后即可调整并保存选区。</p>}</section>
          <section className="mediaSamplerAsr"><div className="mediaSamplerSectionTitle"><span><FileText size={16} />识别与字幕</span></div>{selected ? <><div className="mediaSamplerActions"><button type="button" disabled={Boolean(pending)} onClick={() => void startAsr("txt")}>{pending === "asr:txt" ? <Loader2 className="spin" size={15} /> : <FileText size={15} />}识别文字</button><button className="primary" type="button" disabled={Boolean(pending)} onClick={() => void startAsr("srt")}>{pending === "asr:srt" ? <Loader2 className="spin" size={15} /> : <Clock3 size={15} />}生成 SRT</button></div>{selectedJob && <article className="mediaSamplerTranscript" role={selectedJob.status === "failed" ? "alert" : "status"} aria-live={selectedJob.status === "failed" ? "assertive" : "polite"}><small>{selectedJob.status === "running" ? "本地识别中…" : selectedJob.status === "completed" ? "识别完成" : selectedJob.status === "failed" ? "识别失败" : "等待识别"}</small><p>{selectedJob.error ?? selectedJob.text ?? "识别结果会显示在这里。"}</p>{selectedJob.status === "completed" && <div className="mediaSamplerExports"><button type="button" onClick={() => void exportTranscript("txt")} disabled={Boolean(pending)}><Download size={13} />TXT</button>{selectedJob.output_format === "srt" && <button type="button" onClick={() => void exportTranscript("srt")} disabled={Boolean(pending)}><Download size={13} />SRT</button>}</div>}</article>}</> : <p>选择一条下载历史后即可启动本地 ASR。</p>}</section>
        </aside>
      </main>
      {(message || error) && <footer role={error ? "alert" : "status"} aria-live={error ? "assertive" : "polite"} className={error ? "error" : ""}>{error ?? message}</footer>}
    </section>
  </div>;
}
