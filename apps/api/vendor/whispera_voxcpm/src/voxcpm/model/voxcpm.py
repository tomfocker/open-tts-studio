...
class VoxCPMModel(nn.Module):
    ...
    @staticmethod
    def _normalize_streaming_int(value: int, minimum: int = 1) -> int:
        return max(int(minimum), int(value))

    def _decode_streaming_audio_chunk(
        self,
        pred_feat_seq: List[torch.Tensor],
        emit_patch_count: int,
        streaming_prefix_len: int,
    ) -> torch.Tensor:
        emit_patch_count = self._normalize_streaming_int(emit_patch_count)
        streaming_prefix_len = self._normalize_streaming_int(streaming_prefix_len)

        available_patches = len(pred_feat_seq)
        context_patches = min(
            max(0, streaming_prefix_len - 1),
            max(0, available_patches - emit_patch_count),
        )
        total_patches = emit_patch_count + context_patches
        feat_chunk = torch.cat(pred_feat_seq[-total_patches:], dim=1)
        feat_pred = rearrange(feat_chunk, "b t p d -> b d (t p)")

        patch_len = self.patch_size * self.chunk_size
        decode_audio = self.audio_vae.decode(feat_pred.to(torch.float32))
        if context_patches > 0:
            decode_audio = decode_audio[..., patch_len * context_patches:]
        return decode_audio.squeeze(1).cpu()

    def _stream_audio_from_inference_result(
        self,
        inference_result: Generator[Tuple[torch.Tensor, List[torch.Tensor]], None, None],
        streaming_prefix_len: int,
        streaming_emit_interval: int,
    ) -> Generator[Tuple[torch.Tensor, List[torch.Tensor]], None, None]:
        streaming_prefix_len = self._normalize_streaming_int(streaming_prefix_len)
        streaming_emit_interval = self._normalize_streaming_int(streaming_emit_interval)

        pending_steps = 0
        latest_pred_feat_seq: Optional[List[torch.Tensor]] = None

        for _, pred_feat_seq in inference_result:
            latest_pred_feat_seq = pred_feat_seq
            pending_steps += 1
            if pending_steps < streaming_emit_interval:
                continue

            yield (
                self._decode_streaming_audio_chunk(
                    pred_feat_seq=pred_feat_seq,
                    emit_patch_count=pending_steps,
        ...
        if pending_steps > 0 and latest_pred_feat_seq is not None:
            yield (
                self._decode_streaming_audio_chunk(
                    pred_feat_seq=latest_pred_feat_seq,
                    emit_patch_count=pending_steps,
                    streaming_prefix_len=streaming_prefix_len,
                ),
                latest_pred_feat_seq,
            )
    ...
    def _generate(
        ...
        while retry_badcase_times < retry_badcase_max_times:
            ...
            if streaming:
                for decode_audio, _ in self._stream_audio_from_inference_result(
                    inference_result,
                    streaming_prefix_len=streaming_prefix_len,
                    streaming_emit_interval=streaming_emit_interval,
                ):
                    yield decode_audio
                break
            else:
    ...
    def _generate_with_prompt_cache(
        ...
        while retry_badcase_times < retry_badcase_max_times:
            ...
            if streaming:
                for decode_audio, pred_audio_feat in self._stream_audio_from_inference_result(
                    inference_result,
                    streaming_prefix_len=streaming_prefix_len,
                    streaming_emit_interval=streaming_emit_interval,
                ):
